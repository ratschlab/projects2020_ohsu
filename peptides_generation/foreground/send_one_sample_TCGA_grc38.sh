...
cap=0 #TODO change 
target=juliannetestbug_pya0.17.1_annot_cap${cap}_p_rev
...
for cancer_type in BRCA ; do # TODO add back the OV #OV; do 
			...
			while read sample; do
				...
				out_1=${outdir}/${mutation}_sample_${kmer}mer.pq.gz
					...
					echo "${outdir}/${mutation}${out_file} does not exist"
				       ...
				       spladder_path=/cluster/work/grlab/projects/tmp_tinu/TCGA_for_neoepitopes/TCGA_Breast_1102_conf1_results/splicing/spladder/genes_graph_conf${cf_level}.${sample}
				       cmd_base="immunopepper  build --verbose 1 --output-samples ${sample} --output-dir ${outdir} --ann-path ${annotation} --splice-path ${spladder_path}.pickle --count-path ${spladder_path}.count.hdf5 --ref-path ${genome} --kmer ${kmer} --mutation-mode ${mutation} --somatic ${maf_path} --germline ${vcf_path} --batch-size ${batch_size} --complexity-cap $cap --genes-interest ./tmp_gene" #TODO add whitelist genes --genes-interest ${whitelist_genes}"
