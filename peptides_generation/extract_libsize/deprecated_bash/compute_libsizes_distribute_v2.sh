...
result_file=/cluster/work/grlab/projects/TCGA/PanCanAtlas/immunopepper_paper/peptides_ccell_rerun_gtex_151220/GTEX2019_commit_librarysize_pya.0.17.1_conf2_annot_ref_chrall_cap1000/cohort_mutNone/GTEX_hg38_library_size_75perce_v2.tsv
...
while read sample; do
	...
	echo "${sample}    $(parquet-tools csv ${gene_expre_file} -c ${sample}| tail -n +2 | sort  -n | awk 'BEGIN{c=0} {total[c]=$1; c++;} END{print total[int(NR*0.75)]}')" >> ${result_file}
done < ${sample_file}
