...
run_status='get' #'collect_finished'

if [ $run_status == 'collect_finished' ] ; then  
	echo "collect processed genes to $(dirname ${outfolder})/ref_gene_folder_eq"
	for folder in ${outfolder}/tmp*/gene_expression_detail.pq ; do echo $(dirname $folder)  $(parquet-tools csv -c gene $folder  | cut -f1 -d ',' | grep ENS) ;  done > $(dirname ${outfolder})/ref_gene_folder_eq	
else
	while read gene; do
		 grep -E "${gene}" $(dirname ${outfolder})/ref_gene_folder_eq | sed -z 's/\n/|/g;s/|$/\n/' | cut -f1 -d ' ' | sed "s,$,/${file_out_full},g" >> ./tmp20211015_input_${file_out}_${suffix_out}
