...
while read sample; do 
	outdir=${base_path}/${target}/TCGA_BRCA_normals/${sample}/spladder_confidence_2/annot_frames
        mkdir -p $outdir
	mutation=ref #somatic_and_germline  germline somatic ref ; do  
	out_1=${outdir}/${mutation}_sample_${kmer}mer.pq.gz
		...
		echo "${outdir}/${mutation}${out_file} does not exist"
	       spladder_path=${basedir}/spladder/TCGA_BRCA_Normals/spladder_confidence_2/genes_graph_conf2.${sample}
...
done < ${basedir}/sample_lists/TCGA_matched_normals/list_spladder_TCGA_matched_normals
