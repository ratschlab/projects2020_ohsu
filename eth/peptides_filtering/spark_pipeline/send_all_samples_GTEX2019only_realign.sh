...
parallel=2
#edge_or_segm=edge
suffix="genes3_minPrepr_${mem}_nocast_p1_2"
echo "WARNING check activation myimmuno3"
module load java
export PYSPARK_PYTHON=/cluster/customapps/biomed/grlab/users/prelotla/software/miniconda/envs/pep_test/bin/python
export PYSPARK_DRIVER_PYTHON=/cluster/customapps/biomed/grlab/users/prelotla/software/miniconda/envs/pep_test/bin/python
### Inputs
...
if [ ! -f "${test_output_exist}/_SUCCESS" ] ; then
	...
	#sbatch --exclude=gpu-biomed-[01,04,06,09,10,15-21,23]
#	sbatch --nodelist=${node_file} --job-name=filt${submission_id}_${suffix}_GTEX2019 --cpus-per-task=${parallel} --time=${time_}:00:00 --mem=${mem} -e ${logfile} -o ${output_file} ./tmp_file
fi    
done
