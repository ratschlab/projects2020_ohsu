...
#SBATCH --mem=4G
while read line; do for part in ${line}/*Expr*/*gz ; do new_name=$(echo $part | sed 's,\.gz,\.lz,g') ; cat $part | gzip -d -c | plzip -6 -f -o $new_name ; if [ -f $new_name ] && [ -s $new_name ] ; then echo $part; chmod 440 ${new_name} ; chmod 660 ${part}; rm ${part} ;fi; done; done < files_34.txt
