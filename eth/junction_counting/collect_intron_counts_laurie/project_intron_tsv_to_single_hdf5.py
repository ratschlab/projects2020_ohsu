...
### load data
print(f'loading data from {infname}')
data = pd.read_csv(infname, sep='\t', dtype='str')
...
for chrm in data['chr'].unique():
    for strand in ['+', '-']:
        ...
        c = coords[(coords['chr'] == str(chrm)) & (coords['strand'] == strand)]
        ...
        OUT.create_dataset(name=str(chrm)+':'+str(strand), data=counts, compression='gzip')
OUT.close()
